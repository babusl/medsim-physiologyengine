...
var RATES = {
  iv:       {hr:3.5, sbp:3.5, dbp:2.8, spo2:1.8, etco2:1.8, rr:1.2}, // peak ~10s
  push:     {hr:1.5, sbp:2.2, dbp:1.8, spo2:0.9, etco2:1.0, rr:0.7}, // peak ~20s
...
var DRUGS = {
  ...
  phenylephrine: {
    ...
    effects: function(dose, weight, mode){
      // Pure α1: SVR ↑ → MAP ↑, reflex brady in adults, minimal in peds
      var bpRise = Math.min(35, dose * 0.18);   // ~18 mmHg per 100mcg, ceiling 35
      var hrDrop = mode==='peds' ? Math.round(bpRise*0.1) : Math.round(bpRise*0.4);
      return { sbp: bpRise, dbp: Math.round(bpRise*0.6), hr: -hrDrop };
    },
  ...
  epinephrine_push: {
    ...
    effects: function(dose, weight, mode){
      ...
      // 10mcg push: HR ↑ 15-25, BP ↑ 20-35
      // 50mcg push: HR ↑ 25-40, BP ↑ 35-55
      var intensity = Math.min(1.0, dose / 50);
      return {
        hr:  Math.round(15 + intensity * 30),
        sbp: Math.round(20 + intensity * 40),
        dbp: Math.round(8  + intensity * 15)
      };
    },
